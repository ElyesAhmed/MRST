...
figure;
subplot(1,2,1);
plotCellData(modelWithClog.G, model.rock.poro); hold on;
plotGrid(modelWithClog.G, schedule.control(1).W.cells, 'FaceColor','red','LineStyle','none');
title('Porosity'); axis off tight;

subplot(1,2,2);
plotCellData(modelWithClog.G, log10(model.rock.perm(:,1))); hold on;
plotGrid(modelWithClog.G, schedule.control(1).W.cells, 'FaceColor','red','LineStyle','none');
...
% Plot states
figure; plotToolbar(modelWithClog.G, statesWithClog); title('With Clogging Effects');
figure; plotToolbar(modelWithClog.G, statesNoClog); title('Without Clogging Effects');
figure; plotToolbar(modelWithClog.G, statesNoBact); title('Without Bacterial Effects');
